...
virus <- str_replace(virus, "Phage_", "p")

strains <- readLines("../../strains.ab")
gnmatrix <- read.csv("phage_matrix_p1virus2.tsv", header = T, sep = '\t', row.names = 1)
names(gnmatrix) <- str_replace(names(gnmatrix), "Phage_", "p")
...
pdf("supplfig3.pdf", width = 12, height = 7)
