...
# ST79
df2 <- as.data.frame(readxl::read_excel("st79_p1virus_ifa.xlsx"))
mi_breaks <- factor(c("CC", "CT", "AC", "CA", "AA"))
mi_breaks <- factor(mi_breaks, levels = c("CC", "CT", "AC", "CA", "AA"))
df2 <- df2 %>% pivot_longer(mi_breaks, names_to = "PAM", values_to = "Frequency") %>% group_by(PAM, Frequency) 
df2 <- data.frame(df2)

pam2 <- ggplot(df2, aes(x = Type, y = Frequency, fill = factor(PAM, levels = mi_breaks))) +
  geom_col(position = "dodge") +
  scale_fill_manual(values = c("#80B1D3", "#B3DE69", "#FFED6F", "#FB8072", "#8DD3C7"), breaks = mi_breaks) + theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), text = element_text(size = 12), legend.position = "top",
        legend.text = element_text(size = 8), legend.title = element_text(size = 10)) +
  xlab("CRISPR array") + guides(fill = guide_legend(title.position = "top", title.hjust = 0.5, ncol = 3)) +
  labs(fill = "PAM sequence ")
