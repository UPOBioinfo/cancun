...
mi_breaks <- levels(factor(mdata$CRISPR)) # c("No CRISPR", "I-Fa", "I-Fa/I-Fb", "I-Fa/I-Fb/DgiS1", "I-Fa/DgiS1", "I-Fb", "I-Fb/DgiS1", "DgiS1")
...
#tree1 <- ggtree(tree1, layout = 'rectangular', branch.length = 'variable', size = 1) %<+% heatmap_tabla + # none | variable
#  geom_tiplab2(size = 0)
...
ph1 <- phylo +
  ...
  geom_flag(aes(country = Country, x = x + 0.001), na.rm = T, size = 5.5) +
  theme(legend.position = "none")
#print(ph1)
...
heatmap_table_3$P1virus <- str_replace(heatmap_table_3$P1virus, "DgiS1.i.", "DgiS1")
heatmap_table2_3 <- read.table("metadata_pp.tsv", sep = "\t", header = T, quote = "", stringsAsFactors = FALSE, row.names = 1)[c(2, 3, 4, 5)]
heatmap_table2_3$P1virus <- str_replace(heatmap_table2_3$P1virus, "DgiS1.i.", "DgiS1")
#heatmap_table$Source <- factor(heatmap_table$Source, levels = c("Canada", "Thailand", "China", "Germany"))
...
ph2_3 <- gheatmap(phylo_hide_3, n2_3, offset = -10,  width = 450, font.size = 0, hjust = 1, colnames = F) +
  scale_fill_manual(values = c("#000000"), name = "DgiS1", na.translate = F) +
  theme(legend.position = "none")
#print(ph2_3)
...
ph3_3 <- gheatmap(phylo_hide_3, n3_3, offset = -10,  width = 450, font.size = 0, colnames_angle = 90, hjust = 1, colnames = F) +
  scale_fill_manual(values = c("#FFE5CC", "#5CB85C", "darkorange"), name = "CRISPR-Cas", na.translate = F) +
  theme(legend.position = "none")
#print(ph3_3)
...
ph234_3 <- gheatmap(phylo_hide_3, n_3, offset = -1,  width = 300, font.size = 0, hjust = 1, colnames = F) +
  scale_fill_manual(values = c("black", "#FFE5CC", "#5CB85C", "darkorange", "#8074A8"), name = "DgiS1", na.translate = F) +
...
#legends
phylo2_3 <- cowplot::plot_grid(ph1_3, ph234_3, ph5_3, legends2_3, nrow = 1, rel_widths = c(1, 0.55, 0.2, 0.58))
print(phylo2_3)
...
fig <- ggarrange(ab, cd, nrow = 2, heights = c(1, 1))

pdf("fig4.pdf", width = 14, height = 10)
print(fig)
dev.off()
